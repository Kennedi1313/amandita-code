import {
    Spinner,
    Text, Input, Button, Stack, Flex, Box,
    Select,
    Alert,
    AlertIcon,
    SimpleGrid
...
import { getProducts, getProductsByCategory } from "./services/product-client.js";
...
const Product = () => {
    ...
    return (
        <SidebarWithHeader>
            <Box width={'full'} maxWidth={{ base: 'full', lg: 'container.lg' }} mx="auto">
                {/* Always Visible Category Filter */}
                <Box  paddingTop={'24px'} paddingBottom={'8px'}>
                    <Text fontWeight={'semibold'} fontSize={'4xl'}>Produtos</Text>
                </Box>
                
                <Flex flexDirection={{ base: 'column', md: 'row' }} justify={'space-between'} gap={'1rem'}>
                    ...
                    <Formik
                        ...
                        onSubmit={async (data) => {
                            try {
                                const response = await axios.get(`${getUrl()}/api/v1/products/by-name`, {
                                    params: {
                                        query: data.search,
                                        page: 0,
                                        size: 8,
                    ...
                    >
                        <Form>
                            <Stack
                                ...
                                color={"#5f5482"}
                                style={{ margin: '0', padding: '1.2rem' }}>Pesquisar</Button>
                            </Stack>
                        </Form>
                    </Formik>
                </Flex>
                ...
                {/* Products Display */}
                <Text marginTop={'8px'} textColor={'#5f5482'} fontWeight={'semibold'}>{productListSize} produtos</Text>
