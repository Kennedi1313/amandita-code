      ...
      <Modal isOpen={isOpen} onClose={onClose}>
        ...
        <ModalContent>
          ...
          <ModalBody>
            {selectedSale ? (
              <Box>
                ...
                <Text>
                  <strong>Data:</strong> {formatDate(selectedSale.saleDate)}
                </Text>
                <Text>
                  <strong>Frete:</strong>{" "}
                ...
                <Box pl={4}>
                  ...
                  selectedSale.customer.addresses[0] ? (
                    <Text>
                      {selectedSale.customer?.addresses[0].street},{" "}
                      {selectedSale.customer?.addresses[0].number} -{" "}
                      {selectedSale.customer?.addresses[0].district},{" "}
                      {selectedSale.customer?.addresses[0].city}/RN,{" "}
                      {selectedSale.customer?.addresses[0].zip}
                    </Text>
                  ) : (
                    "Sem Endereço"
                  )}
                      ...
                      {selectedSale.items.length === 0 && (
                        <Tr>
                          <Td
                            ...
                            wordBreak="break-word"
                          >
                            Desculpe o transtorno, os produtos dessa compra
                            foram excluídos do catalogo da loja.
                          </Td>
                        </Tr>
                      )}
          ...
          <ModalFooter gap={4}>
            <Button
              colorScheme="green"
              disabled={selectedSale && selectedSale.status != "APROVADO"}
              onClick={() => changeStatus(selectedSale.id, "PREPARANDO")}
            >
              Preparar Pedido
            </Button>
            <Button
              colorScheme="green"
              disabled={selectedSale && selectedSale.status != "PREPARANDO"}
              onClick={() => changeStatus(selectedSale.id, "ENTREGUE")}
            >
              Entregar Pedido
            </Button>
