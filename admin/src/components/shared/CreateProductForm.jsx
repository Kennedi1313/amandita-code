...
import ProductVariationTable, { prepareVariationsForSave } from '../product/ProductVariationTable.jsx';
...
const generateCombinations = (selectedVariations) => {
    const variationNames = Object.keys(selectedVariations).filter(
        (key) => Array.isArray(selectedVariations[key]) && selectedVariations[key].length > 0
    );
    if (variationNames.length === 0) return [];
    const optionsArrays = variationNames.map((name) => selectedVariations[name].map((value) => ({ [name]: value })));
