...
const upsertVariationByKey = (prevArray, key, patch) => {
    ...
    if (!found) {
        ...
        const options = Object.fromEntries(key.split("|").map(pair => {
...
const ProductVariationTable = ({ variationData = [], generatedCombos = [], setVariationData }) => {
...
return (
    ...
    <Text fontWeight="bold" mb={4}>Combinações geradas</Text>

    {generatedCombos.length === 0 && (
        <Text color="gray.500">Nenhuma combinação possível. Adicione valores nas variações.</Text>
    )}

    <Accordion allowMultiple defaultIndex={[]}>
        {generatedCombos.map((comb) => {
        const comboKey = getComboKey(comb);
        const comboData = variationMap[comboKey] || { options: comb, price: "0", quantity: 0, promo: 0 };

        const valid = Number(comboData.price) > 0 || Number(comboData.quantity) > 0;

        return (
            <AccordionItem key={comboKey} border="1px solid #eee" borderRadius="md" mb={3}>
            <h2>
                <AccordionButton _expanded={{ bg: "purple.50" }}>
                <Flex justify="space-between" align="center" flex="1" textAlign="left" gap={2}>
                    <Text fontWeight="semibold">
                    {Object.entries(comb).map(([k, v]) => `${k}: ${v}`).join(" + ")}
                    </Text>

                    <Tag py={2} minW="fit-content" size="sm" colorScheme={valid ? "green" : "gray"} variant={valid ? "solid" : "subtle"}>
            ...
            <AccordionPanel>
                ...
                <FormLabel htmlFor={`promo-${comboKey}`}>Promoção (%)</FormLabel>
                <Input
                    id={`promo-${comboKey}`}
                    type="number"
                    value={comboData.promo}
                    onChange={(e) => handleChange(comboKey, "promo", e.target.value)}
                />

                <FormLabel htmlFor={`quantity-${comboKey}`}>Quantidade em estoque</FormLabel>
                <Input
                    id={`quantity-${comboKey}`}
                    type="number"
                    value={comboData.quantity}
                    onChange={(e) => handleChange(comboKey, "quantity", e.target.value)}
                />
                </Stack>
            </AccordionPanel>
            </AccordionItem>
