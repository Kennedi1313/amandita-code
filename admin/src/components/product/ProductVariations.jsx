...
const ProductVariations = ({ predefinedVariations, value, onChange }) => {
    ...
    useEffect(() => {
        ...
        variations.forEach((v) => {
            if (v.name) {          // Mantém a variação mesmo que options esteja vazia
    ...
    const removeOption = (vIndex, option) => {
        setVariations((prev) => {
        const updated = [...prev];
        updated[vIndex].options = updated[vIndex].options.filter((o) => o !== option);
        return updated;
        });
    };
  ...
  return (
    <Box>
      ...
      {variations.map((variation, index) => (
        <Box key={`var-${index}`} borderRadius="md" p={4} mb={3} border="1px solid #eee">
          <Flex align="center" mb={4} gap={4}>
            ...
            >
              {predefinedVariations
                .filter(
                  (v) =>
                    !variations.some((vr) => vr.name === v.name) || variation.name === v.name
                )
                .map((v) => (
            ...
            <IconButton icon={<FiTrash />} colorScheme="red" onClick={() => removeVariation(index)} />
          </Flex>
              ...
              <Flex wrap="wrap" gap={2} mb={4}>
                {variation.options.map((option, optIndex) => (
                  <Tag key={`opt-${index}-${optIndex}`} colorScheme="purple">
                    ...
                    <TagCloseButton onClick={() => removeOption(index, option)} />
                  </Tag>
                ))}
              </Flex>

              <Select placeholder="Adicionar valor" onChange={(e) => addOption(index, e.target.value)}>
                {predefinedVariations
      ...
      <Button leftIcon={<FiPlus />} colorScheme="blue" onClick={addVariation} mt={4}>
        Adicionar variação
      </Button>
    </Box>
    );
