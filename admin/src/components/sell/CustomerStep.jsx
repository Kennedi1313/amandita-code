...
import { Stack, Input, Button, FormLabel, Box, Text, useToast, Flex } from '@chakra-ui/react';
    ...
    return (
            <Stack justify={'center'} width={'full'} marginBottom={'10rem'}>
                <Stack spacing={4} width={'full'} marginBottom={8}>
                    <Text fontSize={'xl'}> Adicione um cliente a essa compra ou cadastre um novo cliente </Text>
                    <Input
                        value={cpf}
                        onChange={(e) => setCpf(cpfMask(e.target.value))}
                        placeholder="Digite o CPF"
                    />
                    <Button onClick={handleSearchCustomer} isLoading={loading} colorScheme="blue">
                        Buscar Cliente
                    </Button>
                </Stack>
                <Stack spacing={6}>
                    ...
                    <Stack zIndex={100} position={'fixed'} bottom={0} left={0} width={'full'} backgroundColor={'white'} p={2} alignItems={'center'}>
                        <Flex width={'full'} gap={4} maxWidth={{ base: 'full', lg: 'container.lg' }} flexDirection={{ base: "column-reverse", lg: "row"}}>
                            <Button width="full" bg="gray.500" color="white" _hover={{ bg: 'gray.400' }} onClick={() => setStep(1)}>
                                Voltar
                            </Button>
                            {isRegistered ? (
                                <Button width="full" onClick={() => {onCustomerSelected(customer)}} isLoading={loading} bg={'blue.700'} color={'white'}>
                                    Continuar
                                </Button>
                            ) : (
                                <>
                                    <Button width="full" onClick={() => {onCustomerSelected(customer)}} isLoading={loading} bg={'red.700'} _hover={{ bg: 'red.600' }}color={'white'}>
