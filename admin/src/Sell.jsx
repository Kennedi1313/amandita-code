...
const Sell = () => {
    ...
    const { favoritesDetails, favoritesCount, clearFavorites, addItemToFavorites } = useShoppingFavorites();
    ...
    const handleSearch = async () => {
        try {
            ...
            const response = await fetch(`${getUrl()}/api/v1/products/by-name?query=${searchTerm}&page=${0}&size=${500}`);
    ...
    const handleFinalizeSale = async () => {
        setLoading(true);
        try {
            const saleItemRequests = Object.entries(favoritesDetails).map(([_, product]) => ({
            ...
            toast({
                ...
                description: 'Ocorreu um erro ao processar a venda. Tente novamente mais tarde.',
                status: 'error',
                duration: 5000,
                isClosable: true,
            });
                        ...
                        {searchResults.length > 0 ? (
                            <Stack spacing={4}>
                                {Object.entries(searchResults).map(([index, product]) => (
                                    <Flex key={index} padding={2} borderBottom={'1px solid lightgray'} justifyContent={'space-between'}>
        ...
        <Stack justify={'center'} width={'full'} marginBottom={'8rem'} >
            ...
            {Object.entries(favoritesDetails).map(([index, product]) => (
                <Box key={index} display={'flex'} flexDirection={'column'}>
                    ...
                    <Box borderTop="2px solid" borderColor={'gray.200'} w="95%" alignSelf={'center'} />
                </Box>
            ))}
        </Stack>
    );
    ...
    const renderCustomerInfo = () => (
        <CustomerStep setStep={setStep} onCustomerSelected={(selectedCustomer) => {
    ...
    const renderPaymentMethod = () => (
        <Stack spacing={4} width={'full'} marginBottom={'8rem'} maxWidth={{ base: 'full', lg: 'container.lg' }} >
    ...
    return (
        <SidebarWithHeader>
            ...
            {(step === 1 || step === 3) && (
                <Stack position={'fixed'} bottom={{base: '100px', md: 5}} left={0} width={'full'} backgroundColor={'white'} height={"8rem"}>
                    <Flex 
                        ...
                        <Box display="flex" flexDirection="column">
                            <Box>
                                ...
                                ?   
                                    <span>
                                        <Text fontSize={"xl"}
                                                fontWeight={'bold'}>{ formatCurrency(totalPrice * 0.95) }</Text>
                            ...
                            {totalPrice >= 10000
                            ? <Text className='text-sm font-thin text-gray-700'>ou 3x de {formatCurrency(totalPrice, 0, 3)} sem juros</Text>
                            : totalPrice >= 5000 
                                ? <Text className='text-xs font-thin text-gray-700'>ou 1x de {formatCurrency(totalPrice)} sem juros</Text>
                                : ""
                            }
                        </Box>
                    </Flex>
            ...
            {/* Navigation Buttons */}
            <Stack position={'fixed'} bottom={0} left={0} width={'full'} backgroundColor={'white'} p={2} alignItems={'center'}>
                {step === 1 && (
                    <Flex width={'full'} gap={4} maxWidth={{ base: 'full', lg: 'container.lg' }} flexDirection={{ base: "column-reverse", lg: "row"}} >
                        <Button
                            width={'full'}
                            bg={'gray.500'}
                            color={'white'}
                        ...
                        <Button width="full" bg="blue.700" color="white" _hover={{ bg: 'blue.600' }} onClick={() => setStep(2)}>
                            Próximo
                        </Button>
                    </Flex>
                )}
                ...
                {step === 3 && (
                    <Flex width={'full'} gap={4} maxWidth={{ base: 'full', lg: 'container.lg' }} flexDirection={{ base: "column-reverse", lg: "row"}}>
                        <Button width="full" bg="gray.500" color="white" _hover={{ bg: 'gray.400' }} onClick={() => setStep(step - 1)}>
                            Voltar
                        </Button>
                        <Button width="full" bg="green.700" color="white" _hover={{ bg: 'green.500' }} onClick={handleFinalizeSale} isLoading={loading}>
                            Finalizar Venda
                        </Button>
                    </Flex>
                )}
