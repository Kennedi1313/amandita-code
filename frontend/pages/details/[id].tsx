...
import { getProducts, getProductsById, getProductsByIdNoStore, getProductsNoStore } from '@/lib/productClient';
                ...
                <div className='flex flex-col md:flex-row justify-between items-center space-y-8 container pt-2 
                    ...
                    <div className="relative w-full h-[40rem] overflow-hidden">
                        ...
                        style={{
                            ...
                            >
                            {(props.imagesIds?.length ? props.imagesIds : [{ imageId: props.profileImageId }]).slice().reverse().map((img, i) => (
                                <SwiperSlide key={i}>
                                    <Image
                    ...
                    <div className='flex flex-col max-w-md w-full rounded-md gap-2'>
                        ...
                        <div className='flex flex-col w-full cursor-pointer'>
                            {props.quantity == 0 ?
                                <a href={"https://api.whatsapp.com/send?phone=8498594171&text=Olá,%20tudo%20bem?%20Gostaria%20de%20ser%20avisado%20quando%20este%20produto%20chegar%20em%20estoque:%20https://https://www."+storeInfo.domain+"/details/" + props.id}
                                    target='blank'
                            ...
                            :
                                <div className='rounded-md flex flex-row text-white 
                                    ...
                                    {
                                        isInCart() ? <> <FiShoppingCart className='w-5 h-5 text-white'></FiShoppingCart> Remover do carrinho </>
                                        : <><FiShoppingCart className='w-5 h-5 opacity-100 text-white'></FiShoppingCart> Adicionar ao carrinho </>
                                    }
                                </div>
                            ...
                            <div className='rounded-md border-[1px] border-rose-400 flex flex-row text-white 
                                ...
                                {
                                    isInFavorites() ? 
                                    <><FiHeart className='w-5 h-5 text-white'></FiHeart> Remover dos favoritos</>
                                    : <><FiHeart className='w-5 h-5 opacity-100 text-white'></FiHeart> Adicionar aos favoritos</>
                                }
                            </div>
                        </div>
                    </div>
